...
(function installPatientLiveSync(global){
  ...
  const state={store:live.makeKeyStore({allowMemory:false}),pairingId:'',client:null,lastPlanRevision:'',sent:new Map(),timer:0};
  ...
  function valueStoreForPlan(plan){
    ...
    const prefix='kgg-'+String(plan.i)+'-';
    for(let i=0;i<localStorage.length;i++){
      const key=localStorage.key(i)||'';
      if(!key.startsWith(prefix)||!key.endsWith('-values'))continue;
      try{const value=JSON.parse(localStorage.getItem(key)||'null');if(value&&typeof value==='object')return value;}catch(err){}
    }
    return null;
  }
  ...
  function renderShell(){
    ...
    $('kggLiveJoin').onclick=joinSession;$('kggLiveLeave').onclick=()=>state.client&&state.client.close({reason:'patient_requested'});
    checkStorage();
  }
  ...
  async function handleScannedText(raw){
    ...
    try{
      const parsed=live.parsePairingQr(raw);const result=await live.importPairingQr(raw,state.store,{planRef:String((localPlan()&&localPlan().i)||'')});state.pairingId=result.pairingId;
  ...
  async function receiveMessage(message){
    ...
    try{if(global.KGGPatientPlanImport&&typeof global.KGGPatientPlanImport.replaceConfirmed==='function')global.KGGPatientPlanImport.replaceConfirmed(raw);setStatus('Plan aktualisiert. Trainingswerte bleiben lokal erhalten.','ok');}catch(err){setStatus('Plan konnte nicht übernommen werden.','warn');}
  }
