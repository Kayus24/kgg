...
| G03 | Normales ChatGPT entdeckt das Plugin und ruft mindestens ein read-only Tool auf. | Fresh Discovery plus echter read-only Tool-Canary mit E3-Evidence. | E3: Discovery, Tool-Request, Tool-Response | `PARTIAL` (`DISCOVERY_VISIBLE_EXECUTION_UNVERIFIED`) | Die Detailseite lädt und zeigt die Tools; der aktuelle normale-ChatGPT-Tool-Call fehlt noch. Nur mit separatem Kosten-/Kommunikations-Gate ausführen; sonst Custom-GPT-Fallback behalten. | [G03](03-chatgpt-plugin-connection.md) |
