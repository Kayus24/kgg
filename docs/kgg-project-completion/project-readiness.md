...
| G03 | Normales ChatGPT entdeckt das Plugin und ruft mindestens ein read-only Tool auf. | Fresh Discovery plus echter read-only Tool-Canary mit E3-Evidence. | E3: Discovery, Tool-Request, Tool-Response | `PARTIAL` (`DISCOVERY_VISIBLE_TOOL_CALL_NOT_OBSERVABLE`) | Die Detailseite lädt und zeigt 11 Tools; der ausdrücklich autorisierte normale-ChatGPT-Probe-Aufruf für `get_current_state(actor=system)` antwortete `NOT_OBSERVABLE`. Erst eine geänderte Host-/Plugin-Bindung rechtfertigt einen neuen Einzel-Call; bis dahin Custom-GPT-Fallback behalten. | [G03](03-chatgpt-plugin-connection.md) |
