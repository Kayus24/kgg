...
Der aktuelle technische Zustand wird nie aus diesem Dokument abgeleitet. Main, Code, Branches, PRs, Workflows, Runs, Jobs, Artefakte und Tests werden frisch aus GitHub gelesen. Dauerhafte fachliche Entscheidungen werden aus dem privaten Project Memory geladen. GPT-/Plugin-Knowledge ist nur Retrieval-Hilfe. Der Editor-Snapshot ist eine getrennte externe Evidenzquelle.
...
Ein früheres Approval gilt nicht automatisch für einen anderen SHA, Candidate, Scope oder eine andere Konsequenz. Jede Freigabe ist an das konkret benannte Ziel gebunden. Ein sichtbarer Button, ein Modelllabel oder ein plausibel aussehender Code ist kein Erfolgsbeleg.

## 4. Source of Truth und Fresh State

GitHub ist führend für technischen Zustand: Main-SHA, Code, Manifeste, Action-Schemas, PRs, Workflows, Runs, Jobs, Artefakte und Tests. Das private Project Memory ist führend für bestätigte Entscheidungen, Begründungen, Handoffs und dauerhafte Lessons. Knowledge-Packs beschleunigen Retrieval, ersetzen aber niemals Live-Reads. Der GPT-Editor und sein Snapshot bleiben eine separate externe Evidenzquelle.

Vor jeder Phase mit Repository-, Ticket-, Editor- oder Laufbezug frisch lesen:

- origin/main, 40-stellige SHA, Branch, HEAD und Worktree-Status;
...
- bei Editorfragen den externen Editor-Snapshot.
...
READ_ONLY_PREFLIGHT -> BASELINE_CONFIRMED -> FOLLOWUP_CANDIDATE -> FOLLOWUP_TESTING -> FOLLOWUP_REVIEW_READY -> WAITING_MERGE_APPROVAL -> POST_MERGE_REFRESH -> CANARY_READY -> CANARY_RUNNING -> CANARY_RECONCILING -> CANARY_PASS oder CANARY_BLOCKED -> GOAL_CANONICALIZATION -> PLUGIN_MAPPING -> PLUGIN_SKELETON -> PLUGIN_READONLY -> AB_READY -> AB_RUNNING -> FAULT_INJECTION -> ROUND_1 -> ROUND_2 -> MIGRATION_GATE.

BLOCKED und STALE_CONTEXT sind harte Zustände. Ein Zustand darf nur nach seinen Eingangskriterien verlassen werden. Der lokale Plugin-/Supervisor-Zustand darf den GitHub-Action-Vertrag nicht erweitern. Ein Canary hat genau einen Dispatch; bei Timeout, Transportfehler, fehlender Sichtbarkeit oder unklarer Antwort niemals redispatchen, sondern zuerst vorhandene Runs reconciliieren.
...
- im definierten Run-Namen ausschließlich das exakt getrimmte Segment nach dem letzten |.
...
Ein Canary meldet mindestens Run-ID, head_sha, Jobstatus, Source, Tooling, Validation, Result, Upload, Artefaktstatus, Error-Class und Workflow-Conclusion. Die Ebenen müssen semantisch zusammenpassen. Produkttest rot bei erfolgreicher Infrastruktur bedeutet CANARY_INFRASTRUCTURE=PASS und ISSUE_CURRENT_REPRODUCTION=FAIL. Tooling-/CI-Fehler bedeuten INFRASTRUCTURE=PASS für die Fehlerpropagation, aber ISSUE_RESULT=UNDETERMINED. Jede Inkonsistenz ist CANARY=FAIL und stoppt weitere Dispatches.

## 8. Goal-Kanonisierung

Dieser Vertrag ist die einzige führende Goal-Datei. Bestehende fachliche und technische Dokumente bleiben in ihren Zuständigkeiten und werden hier referenziert, nicht kopiert. Generated Knowledge, Drive/Memory, Editor-Instructions und Plugin-References dürfen auf diesen Vertrag verweisen, aber keine konkurrierende Vollversion erzeugen.
...
| Editor-Sync | kein Plugin-Write | externer Snapshot | DEFER |
...
Der Candidate ist reversibel und enthält zunächst nur Vertrag, Referenzen, read-only Skills/Adapter, Evaluations-Harness und Tests. Zulässige erste Fähigkeiten:
...
Jeder Skill benennt Zuständigkeit, zulässige Reads, verbotene Writes, Stop-Gates und seine Belege. Vertrauenswürdige Runtime/MCP-Gates erzwingen Actor-Binding, Lease, Berechtigung, Fresh-SHA, Replay-Schutz, Evidence-Integrität und Append-only-Regeln; Skill-Text allein ist keine Sicherheitsgrenze.
...
Der Bruder darf Root Cause prüfen, Evidenzlücken markieren, Tests verlangen, einen Fixplan oder eine Goal-Anpassung vorschlagen und einen strengeren Stop empfehlen. Sein Ergebnis ist ADVISORY. Er darf nicht automatisch schreiben, dispatchen, mer­gen, Main/Live/Patient ändern, weitere GPTs rekursiv öffnen, Safety-Regeln abschwächen oder Erfolg ohne Beleg behaupten. Browser-Fallback ist ausschließlich Transport; er ist kein neuer Write-Vertrag. Maximal ein frischer Transport-Retry mit neuer Generation/Revision und unverändertem Requirements-Hash.

## 12. Fairer A/B/C-Benchmark

A = CUSTOM_GPT_PRODUCTION (Control), B = PLUGIN_CANDIDATE, C = CODEX_PLUS_PLUGIN. Alle drei erhalten denselben Fresh Main, denselben Goal-Vertrag, denselben #180-Auftrag, dieselben Read-Rechte, Safety-Grenzen, Expected Outcomes und Metrikdefinitionen. Kein Pfad erhält versteckte Assertions oder einen Informationsvorteil.

Zu messen sind mindestens reads, context_items, clarifying_questions, tool_calls, action_calls, dispatches, duplicate_dispatches, runtime_ms, failure_classes, root_cause_quality, result_quality, evidence_quality, source_fidelity, stale_state_errors, safety_violations, unauthorized_writes, unnecessary_context, unnecessary_reads und unnecessary_tool_calls. Surface-Capability-Unterschiede werden separat als SURFACE_CAPABILITY_DIFFERENCE dokumentiert und nicht als Intelligenzunterschied interpretiert.

Der #180-Benchmark bewertet mindestens Fresh-State-Treue, Reproduktion, Root Cause, Scope-Treue, Stop-Verhalten, Evidenzqualität, Kosten-/Laufzeitdaten und unerlaubte Writes. Ein Testlauf ohne belastbare Messwerte ist NOT_MEASURED, nicht geschätzt.

## 13. Fault Injection und Kontrollschleifen

Nach einem funktionierenden A/B/C-Pilot werden mindestens diese Fehler injiziert: stale Main-SHA, falsche oder fehlende head_sha, Prefix-/Suffix-Kollision, mehrere ähnliche Runs, Duplicate Request, Browser-/Tool-Timeout, CI-Tooling-Failure, fehlendes Artefakt, Artifact/Workflow-Widerspruch, MCP nicht verfügbar, fehlender Skill, stale Reference, Bruder nicht verfügbar, unerlaubter Goal-Änderungsvorschlag und unerlaubter Write-Versuch.
...
- CROSS_SURFACE_PASS: vorgesehene ChatGPT-/Codex-Oberflächen funktionieren real; Codex-only Hooks bleiben gekennzeichnet.
...
Der historische Read-only-Härtungs- und Merge-Schritt ist abgeschlossen. Der Post-Merge-Canary muss als eigene Run-Evidenz behandelt werden und nicht als Goal- oder Plugin-Erfolg. Vor dem Übergang in PLUGIN_MAPPING ist diese Datei als einzelner kanonischer Vertrag in einem Candidate-Branch zu reviewen; danach sind Mapping, Skeleton und read-only Fähigkeiten separat zu testen. Der offene Issue-#180-Befund bleibt ein Produkt-/UI-Regressionsergebnis und darf nicht als Infrastrukturfehler umgedeutet oder ohne eigenen Fix-Loop geschlossen werden.
