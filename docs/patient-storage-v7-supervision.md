...
- Beim zweiten `validate_only`-Dispatch (`patient-storage-v7-hotfix-20260729-r2`) entfernte Windows PowerShell beim nativen `-f payload_json=...`-Aufruf die inneren JSON-Anführungszeichen. Die lokale Validierung war grün, GitHub erhielt aber kein gültiges JSON. Weitere Dispatches nutzen deshalb den JSON-STDIN-Modus der GitHub CLI.
