  ...
  function onAnimatedReorderMove(ev){
    ...
    const target=KGGSharedReorder.targetIndex(cards,floatingMid);
  ...
  function finishAnimatedReorder(ev){
    ...
    let to=Number.isInteger(press.targetIndex)?press.targetIndex:Array.from(press.list.children).indexOf(press.placeholder);
    if(!Number.isInteger(press.targetIndex)&&to>from)to-=1;
    to=Math.max(0,Math.min(state.plan.length-1,to));
    const moved=to!==from;
    if(moved){
      const next=KGGSharedReorder.move(state.plan,from,to);
      if(!next){cleanupAnimatedReorder(true);renderPlan();return;}
      state.plan=next;
