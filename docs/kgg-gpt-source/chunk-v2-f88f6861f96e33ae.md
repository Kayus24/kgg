  ...
  function convertKggH2PayloadToPatientPayload(raw){
    ...
    return {
      ...
      meta:{planId:data.i||'',title:data.t||'KGG Trainingsplan',source:data.m&&data.m.schema==='KGGH3'?'KGGH3':'KGGH2',media:data.m&&data.m.media||null}
    };
  }
  function buildPatientShareFromCurrentPlan(planOverride,options){
    ...
    const publicPayload=buildKggH3PayloadFromPlan(plan,exercises,patient,mediaMeta);
    ...
    const patientUrl=shareable?makeKggH3ShareUrl(patientBaseUrl,publicPayload):'';
    return {url:patientUrl,debugUrl,payload,publicPayload,plan,shareable};
  }
  function tryRenderQrCode(url,boxId,statusId,options){
    const settings=options||{}, box=$(boxId||'patientQrBox'), status=$(statusId||'patientQrStatus');
    if(!box||!status)return;
    const cellSize=Number(settings.cellSize)||10, margin=settings.margin===undefined?4:Number(settings.margin), errorCorrection=settings.errorCorrection||'M', alt=settings.alt||'QR-Code zur Patienten-App';
    box.innerHTML=''; status.textContent='';
    try{
      let imgData='';
      if(window.KGGQrCore&&typeof window.KGGQrCore.renderQrToImg==='function')imgData=window.KGGQrCore.renderQrToImg(url,{cellSize,margin,errorCorrection});
      else if(typeof window.qrcode==='function'){const qr=window.qrcode(0,errorCorrection); qr.addData(url); qr.make(); imgData=qr.createDataURL(cellSize,margin);}
      if(imgData){const img=document.createElement('img'); img.alt=alt; img.src=imgData; box.appendChild(img); return;}
    }catch(err){console.warn('QR konnte nicht gerendert werden:',err);}
    box.innerHTML='<span class="qrStatus">QR fehlgeschlagen. Link nutzen.</span>'; status.textContent='Link nutzen.';
  }
  ...
  function currentPatientRootUrl(){
    const link=$('patientRootAppLink');
    return link&&link.href&&link.href!=='#'?link.href:'';
  }
  ...
  async function copyPatientRootLink(){
    const status=$('patientRootQrStatus'), url=currentPatientRootUrl();
    if(!url){if(status)status.textContent='Keine App-Adresse.';return;}
    try{
      const ok=await copyTextValue(url);
      if(ok){if(status)status.textContent='App-Adresse kopiert.';return;}
    }catch(err){console.warn('App-Adresse konnte nicht kopiert werden:',err);}
    const field=$('patientRootAppAddress');
    if(field){field.focus();field.select();}
    if(status)status.textContent='Kopieren blockiert. Adresse ist markiert.';
  }
  ...
  async function renderPatientShareOutput(options){
    ...
    const link=$('patientAppLink'), notice=$('patientShareNotice'), box=$('patientQrBox'), status=$('patientQrStatus'), copyBtn=$('copyPatientLink'), rootFallback=$('patientRootFallback'), rootLink=$('patientRootAppLink'), rootAddress=$('patientRootAppAddress'), rootCopy=$('copyPatientRootLink');
    ...
    if(!share.shareable){
      if(rootFallback)rootFallback.hidden=true;
    ...
    const rootUrl=patientRootAppUrl();
    if(rootFallback)rootFallback.hidden=!rootUrl;
    if(rootUrl){
      if(rootLink){rootLink.href=rootUrl;rootLink.textContent='Kurze Patienten-App-Adresse öffnen';}
      if(rootAddress)rootAddress.value=rootUrl;
      if(rootCopy)rootCopy.classList.remove('hidden');
      tryRenderQrCode(rootUrl,'patientRootQrBox','patientRootQrStatus',{cellSize:6,margin:2,errorCorrection:'M',alt:'Kleiner QR-Code nur für die öffentliche Patienten-App-Adresse'});
    }
    return share.url;
  }
  ...
  function resetFinishModal(){
    ...
    const rootFallback=$('patientRootFallback'), rootLink=$('patientRootAppLink'), rootAddress=$('patientRootAppAddress'), rootBox=$('patientRootQrBox'), rootStatus=$('patientRootQrStatus'), rootCopy=$('copyPatientRootLink');
    if(rootFallback)rootFallback.hidden=true;
    if(rootLink)rootLink.href='#';
    if(rootAddress)rootAddress.value='';
    if(rootBox)rootBox.innerHTML='<span class="qrStatus">App-QR wird vorbereitet …</span>';
    if(rootStatus)rootStatus.textContent='';
    if(rootCopy)rootCopy.classList.add('hidden');
  }
  ...
  function decodePatientPayloadFromHash(){
    ...
    const publicMatch=hash.match(/^#(KGGH[23]):(.+)$/i);
    if(publicMatch){
      try{
        const code=publicMatch[1].toUpperCase()+':'+publicMatch[2];
        const raw=window.KGGPlanFormat?decodeKggPlanCode(code):decodeKggJsonBase64Url(publicMatch[2]);
        return convertKggH2PayloadToPatientPayload(raw);
      }catch(err){console.warn(publicMatch[1].toUpperCase()+' Patienten-Link konnte nicht gelesen werden:',err); return {error:true};}
    }
