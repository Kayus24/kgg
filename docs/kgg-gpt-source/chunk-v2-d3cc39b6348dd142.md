  ...
  function makeUrlWithPayload(baseUrl,payload){
    ...
    const publicPayload=payload&&Array.isArray(payload.e)?payload:buildKggH3PayloadFromInternalPayload(payload);
    return base+'?plan='+encodeURIComponent(encodeKggH3Payload(publicPayload));
  }
  ...
  window.KGGTicket015PatientMedia={manifest:buildExerciseMediaManifestForPatient,refs:buildExerciseMediaRefsForPatient};
  ...
  function buildPlanMediaMeta(rawExercises,ttlSeconds){
    const items=mediaItemsFromExercises(rawExercises||[]).map(entry=>entry.media);
  ...
  function buildKggH3PayloadFromPlan(plan,exercises,patient,mediaMeta){
    const payload=buildKggH2PayloadFromPlan(plan,exercises,patient,mediaMeta);
    payload.m={...(payload.m||{}),schema:'KGGH3'};
    return payload;
  }
  function buildKggH3PayloadFromInternalPayload(payload){
    const p=payload||{};
    const meta=p.meta||{};
    const plan={id:meta.planId||p.id||'',title:meta.title||'KGG Trainingsplan',days:meta.days||6};
    return buildKggH3PayloadFromPlan(plan,Array.isArray(p.plan)?p.plan:[],p.patient||{},meta.media||null);
  }
  function encodeKggH3Payload(publicPayload){
    if(!window.KGGPlanFormat||typeof window.KGGPlanFormat.encodeKggH3!=='function')throw new Error('Lokaler KGGH3-Encoder fehlt.');
    return window.KGGPlanFormat.encodeKggH3(publicPayload);
  }
  function decodeKggPlanCode(code){
    if(!window.KGGPlanFormat||typeof window.KGGPlanFormat.decodeCode!=='function')throw new Error('Lokaler KGGH2/KGGH3-Decoder fehlt.');
    return window.KGGPlanFormat.decodeCode(code).raw;
  }
  ...
  function makeKggH3ShareUrl(baseUrl,publicPayload){
    const base=String(baseUrl||'').split('#')[0].split('?')[0];
    return base+'?plan='+encodeURIComponent(encodeKggH3Payload(publicPayload));
  }
  function patientRootAppUrl(){
    const base=String(patientBaseUrl||'').split('#')[0].split('?')[0];
    return isShareablePatientBaseUrl(base)?base:'';
  }
  function expandKggH2Exercise(item){
    const e=Array.isArray(item)?item:[];
    const progressionSelection=e[7]&&typeof e[7]==='object'&&!Array.isArray(e[7])&&e[7].k==='kgg015'?e[7]:null;
    const media=progressionSelection?[]:(Array.isArray(e[7])?e[7]:(e[7]?[e[7]]:[]));
