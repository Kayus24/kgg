  ...
  // Narrow bridge for the modular progression editor. It keeps the existing
  // catalog, plan-store and persistence functions inside this runtime closure.
  if((window.KGG_ROLLOUT_PROFILE||'colleague').toLowerCase()==='admin')window['KGGProgressionEditorBridge']={
    catalog:()=>bank.map(item=>({...item,media:ensureExerciseMediaList(item)})),
    upsert:(exercise,reason)=>upsertPlanExerciseToBank(exercise,reason||'ticket_015_progression'),
    sync:reason=>syncStatePlanToStore(reason||'ticket_015_progression'),
    ...
    readMedia:async media=>{
      ...
      return patientDecryptMedia(media,record.blob);
    }
  };
