  ...
  function syncPlanFromTextInput(reason,options){
    if(state.textSyncing)return;
    const input=$('exerciseInput');
    resizeExerciseInputToContent();
    const text=input&&input.value||'';
    if(hasIncompleteTextExerciseSegment(text,state.planText,options)){
      // The text remains the live master even while a small partial edit is
      // protected from replacing the last valid structured plan.
      state.planText=text;
      save();
      return;
    }
    ...
    if(structured!==null)state.plan=structured.map(ensureUiExerciseShape);
      ...
      state.plan=segments.map((segment,index)=>parseTextExercise(segment,findExistingForTextSegment(segment,index,usedExistingIds))).filter(Boolean).map(ensureUiExerciseShape);
    }
  ...
  function sameParsedTextExercise(left,right){
    if(!left||!right||compactTextExerciseName(left.name||left.rawText)!==compactTextExerciseName(right.name||right.rawText))return false;
    const fields=['startMetric','startLoad','unit','metricUnit','weightUnit','loadUnit'];
    return fields.every(field=>String(left[field]||'')===String(right[field]||''));
  }
  function restorePlanFromSavedLiveText(){
    const text=String(state.planText||'');
    if(!text.trim())return false;
    // Keep the exact saved field value in both cases.  A valid saved master
    // repairs a stale/collapsed persisted state; an incomplete one keeps the
    // previous valid state intact until the therapist finishes editing it.
    if(hasIncompleteTextExerciseSegment(text,''))return true;
    const structured=structuredExercisesFromPlanText(text);
    const restored=structured!==null
      ?structured.map(ensureUiExerciseShape)
      :(()=>{const usedExistingIds=new Set(); return splitPlanText(text).map(part=>part.text.trim()).filter(Boolean).map((segment,index)=>parseTextExercise(segment,findExistingForTextSegment(segment,index,usedExistingIds))).filter(Boolean).map(ensureUiExerciseShape);})();
    const current=Array.isArray(state.plan)?state.plan:[];
    const differs=restored.length!==current.length||restored.some((exercise,index)=>!sameParsedTextExercise(exercise,current[index]));
    if(restored.length&&differs){
      state.plan=restored;
      state.liveDraftId=null;
      // Persist only a real valid recovery. Incomplete live text keeps its
      // previous valid structured plan and must never rewrite local storage.
      save();
    }
    return true;
  }
  function restoreSavedLiveTextInput(){
    const input=$('exerciseInput');
    if(!input)return;
    const text=String(state.planText||'');
    state.textSyncing=true;
    input.value=text;
    state.textSyncing=false;
    resizeExerciseInputToContent();
  }
  ...
  function applySelectedExerciseToText(ex,options){
    ...
    if(mode==='append'){
      ...
      syncPlanFromTextInput('ui_select_db_exercise_append_to_text',{finalizeShortTextEdit:true});
    ...
    input.setSelectionRange&&input.setSelectionRange(caret,caret);
    state.bankOpen=keepBankOpen;
    syncPlanFromTextInput('ui_select_db_exercise_into_text',{finalizeShortTextEdit:true});
    restoreDbScrollAnchor(scrollAnchor);
    if(keepFocus)keepExerciseInputFocus(caret);
  ...
  function confirmExerciseInput(){syncPlanFromTextInput('ui_confirm_text_master',{finalizeShortTextEdit:true});}
