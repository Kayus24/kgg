<script>
(function(){'use strict';
  const VERSION='KGG_GITHUB_UPDATE_v092_ticket_015_admin_stage_release_fixes';
  ...
  const KGG_BUILD_INFO={"release":"v092","buildTime":"2026-09-13T19:43:16Z","buildCode":"module-v092-ticket-015-admin-stage-release-fixes","htmlFile":"kgg-update/index.html"};
  ...
  function allExerciseMediaList(ex){
    const direct=ensureExerciseMediaList(ex);
    const variants=Array.isArray(ex&&ex.progressionVariants)?ex.progressionVariants.flatMap(variant=>ensureExerciseMediaList(variant)):[];
    return direct.concat(variants);
  }
  function mediaItemsFromExercises(exercises){
    return (exercises||[]).flatMap(ex=>allExerciseMediaList(ex).map(media=>({ex,media})));
  }
  ...
  async function mediaItemsForBundle(exercises){
    ...
    for(const ex of (exercises||[])){
      for(const media of allExerciseMediaList(ex)){
  ...
  window.KGGTicket015PatientMediaBundleReady=()=>!!lastPatientMediaBundleManifest;
