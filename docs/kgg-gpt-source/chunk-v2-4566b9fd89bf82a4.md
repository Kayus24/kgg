      const expectedPrefix=profile==='admin'?'KGG_ANDROID_ADMIN_':'KGG_ANDROID_KOLLEGEN_';
      return match[2].startsWith(expectedPrefix)?match[1]:'';
    }catch(err){return '';}
  }
  function currentKggVersionIdentity(){
    const sourceCode=parseKggSourceVersionCode(VERSION);
    const node=document.getElementById('kgg-source-truth');
    if(!sourceCode||!node)return null;
    try{
      const data=JSON.parse(node.textContent||'{}');
      const current=data&&data.currentVersion;
      const metadataCode=current&&current.versionCode;
      const versionName=parseKggVersionName(current&&current.versionName);
      if(!Number.isSafeInteger(metadataCode)||metadataCode!==sourceCode||!versionName)return null;
      if(versionName.major!==1||versionName.minor!==0||versionName.patch!==sourceCode)return null;
      return {sourceCode,versionName};
  ...
  function githubUpdateTargetFromManifest(manifest){
    ...
    const channel=manifest.channels&&typeof manifest.channels==='object'&&manifest.channels[profile]&&typeof manifest.channels[profile]==='object'?manifest.channels[profile]:null;
    if(channel&&channel.profile&&channel.profile!==profile)return null;
    const latestVersion=channel&&channel.versionName||(profile==='admin'?manifest.latestAdminVersion:manifest.latestColleagueVersion);
    const latestUrl=channel&&channel.url||(profile==='admin'?manifest.adminUrl:manifest.colleagueUrl);
    const explicitReleaseId=channel&&channel.releaseId||(profile==='admin'?manifest.latestAdminReleaseId:manifest.latestColleagueReleaseId)||(profile==='colleague'?manifest.latestVersion:'');
    const urlReleaseId=kggWebReleaseIdFromUrl(latestUrl,profile);
    const releaseId=explicitReleaseId||urlReleaseId;
    const current=currentKggVersionIdentity();
    const latest=parseKggVersionName(latestVersion);
    if(!current||!latest||!urlReleaseId||parseKggWebReleaseId(releaseId)===null||String(releaseId)!==urlReleaseId)return null;
    if(!isKggHttpsAssetUrl(latestUrl,'.html'))return null;
    if(compareKggVersionNames(latest.raw,current.versionName.raw)!==1)return null;
    return {version:latest.raw,webReleaseId:String(releaseId),url:latestUrl,notes:channel&&channel.notes||manifest.releaseNotes||manifest.notes||''};
  }
  ...
  function androidApkUpdateTargetFromManifest(manifest){
    if(!isNativeAndroidShell()||!manifest||typeof manifest!=='object')return null;
    const profile=currentRolloutProfile();
    const latestVersion=manifest.latestAndroidShellVersion||'';
    ...
    const latestShell=parseKggAndroidShellVersion(latestVersion);
    const urlShell=kggAndroidShellFromApkUrl(latestUrl,profile);
    if(latestShell===null||!isKggHttpsAssetUrl(latestUrl,'.apk')||urlShell!==latestVersion||!/^[0-9a-f]{64}$/.test(String(latestSha||'')))return null;
    const nativeStatus=nativeAppUpdateStatus();
    const currentShell=parseKggNativeShellCode(nativeStatus.currentShellVersion);
    if(currentShell===null||latestShell<=currentShell)return null;
    return {version:latestVersion,androidShellVersion:latestVersion,url:latestUrl,sha256:latestSha,notes:manifest.releaseNotes||manifest.notes||''};
  }
  ...
  function stripExerciseName(raw){return String(raw||'').replace(/\d+(?:[,.]\d+)?\s*(kg|kilo|wdh|wh|reps|min|sek|sec|s)\b/ig,' ').replace(/\b(a|x|mal)\b/ig,' ').replace(/\s+/g,' ').trim();}
