  ...
  function updateTabletLayoutHandle(){
    ...
    /* The handle is inside the transformed tablet app. Fixed-position
       descendants of a transformed ancestor use that ancestor as their
       containing block, so convert the viewport target to app-local
       coordinates before writing the inline position. */
    handle.style.left=Math.round(center-rect.left-(handleWidth/2))+'px';
    handle.style.top=Math.round(8)+'px';
