  ...
  $('exerciseInput').addEventListener('input',()=>upsertLiveExerciseFromText()); $('exerciseInput').addEventListener('change',()=>syncPlanFromTextInput('ui_textfield_master_change',{finalizeShortTextEdit:true})); $('exerciseInput').addEventListener('focus',()=>render()); $('clearInput').onclick=()=>{clearInputAndRemoveLiveTextExercises();};
  ...
  $('copyPatientRootLink').onclick=copyPatientRootLink;
  $('deleteExercise').onclick=()=>{if(externalEditorContext){const context=externalEditorContext,ex=context.exercise,accepted=typeof context.onDelete==='function'?context.onDelete(ex)!==false:false;if(accepted)closeEditor();return;}if(state.editId){if($('deleteExercise').dataset.scope==='bank')openBankDeleteModal(state.editId); else removeExercise(state.editId);} closeEditor();};
