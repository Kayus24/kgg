const KGG_SW_VERSION = 'kgg-therapist-v365-finish-modal-output-buttons';
const KGG_CACHE = KGG_SW_VERSION + '-app-shell';
const KGG_SHELL = [
  './',
  './KGG_APP_ADMIN_v365_finish_modal_output_buttons.html',
  './KGG_APP_KOLLEGEN_v365_finish_modal_output_buttons.html',
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith(
    fetch(request)
      ...
      .catch(() => caches.match(request).then(cached => cached || caches.match('./KGG_APP_KOLLEGEN_v365_finish_modal_output_buttons.html')))
  );
});
