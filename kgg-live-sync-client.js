...
(function installKggLiveSync(global){
  ...
  function makeKeyStore(options){
    ...
    async function removeByPlanRef(planRef){
      const target=String(planRef||'');
      if(global.indexedDB){const database=await db();await new Promise((resolve,reject)=>{const tx=database.transaction(KEY_STORE,'readwrite'),store=tx.objectStore(KEY_STORE),request=store.openCursor();request.onsuccess=()=>{const cursor=request.result;if(!cursor)return;const value=cursor.value;if(value&&String(value.planRef||'')===target)cursor.delete();cursor.continue();};request.onerror=()=>reject(request.error||failure('SECURE_STORAGE_UNAVAILABLE','Kopplungen konnten nicht gelöscht werden.'));tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error||failure('SECURE_STORAGE_UNAVAILABLE','Kopplungen konnten nicht gelöscht werden.'));});return;}
      for(const [id,value] of memory.entries())if(value&&String(value.planRef||'')===target)memory.delete(id);
    }
    async function clearAll(){
      if(global.indexedDB){const database=await db();await new Promise((resolve,reject)=>{const tx=database.transaction(KEY_STORE,'readwrite');tx.objectStore(KEY_STORE).clear();tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error||failure('SECURE_STORAGE_UNAVAILABLE','Kopplungen konnten nicht zurückgesetzt werden.'));});return;}
      memory.clear();
    }
    return {
      ...
      remove,removeByPlanRef,clearAll,
