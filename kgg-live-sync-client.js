...
(function installKggLiveSync(global){
  ...
  class LiveSession{
    ...
    async receiveControl(value){
      ...
      if(!this.key&&!this.nativeSession){this.assertSessionActive();const binding=await buildPairingBinding(data=>this.sign(data),this.pairingId);this.assertSessionActive();if(this.cryptoBridge){assert(typeof this.cryptoBridge.deriveSessionKey==='function','NATIVE_CRYPTO_REQUIRED','KGGLiveKey-Bridge unterstützt keine Sitzungsschlüsselableitung.');const result=await this.cryptoBridge.deriveSessionKey({curve:'P-256',sessionId:this.session.sessionId,sessionSalt:this.session.sessionSalt,pairingId:this.pairingId,pairingBinding:base64UrlEncode(binding),privateKeyHandle:this.nativePrivateHandle,peerPublicKey:value.publicKey,role:this.role});this.assertSessionActive();this.nativeSession=opaqueHandle(result&&(result.sessionHandle||result.keyHandle||result.handle||result));}else{this.assertSessionActive();this.key=await deriveSessionKey(this.privateKey,value.publicKey,{...this.session,pairingBinding:binding});this.assertSessionActive();}this.setStatus('ready',{keyReady:true});await this.flushQueue();this.assertSessionActive();await this.onReady(this.status());}
    }
    async send(type,payload){
      ...
      const inner={type,...(payload||{})};assertSynthetic(this.config,inner);if(type!=='close')assert(this.sendSequence<MAX_DATA_FRAMES,'QUOTA','Sitzungs-Frame-Limit erreicht.');const sequence=++this.sendSequence;let frame;this.assertSessionActive();if(this.nativeSession){assert(typeof this.cryptoBridge.encryptFrame==='function','NATIVE_CRYPTO_REQUIRED','KGGLiveKey-Bridge unterstützt keine Frame-Verschlüsselung.');const result=await this.cryptoBridge.encryptFrame({v:PROTOCOL_VERSION,sessionId:this.session.sessionId,role:this.role,sender:this.role,sequence,payload:inner,sessionHandle:this.nativeSession});this.assertSessionActive();frame=result&&result.frame||result;validateEnvelopeShape(frame,this.role);}else{this.assertSessionActive();frame=await encryptEnvelope(this.key,this.session.sessionId,this.role,sequence,inner);this.assertSessionActive();}assert(!this.sentNonces.has(frame.nonce),'NONCE_REUSE','Nonce wurde wiederverwendet.');this.sentNonces.add(frame.nonce);this.assertSessionActive();await this.queue.put(this.session.sessionId,frame,new Date(this.session.expiresAt).getTime());this.assertSessionActive();
