...
(function installKggLiveSync(global){
  ...
  async function createEphemeralKeyPair(){
    const keyPair=await cryptoObject().subtle.generateKey({name:'ECDH',namedCurve:'P-256'},true,['deriveBits']);
  ...
  async function deriveSessionKey(privateKey,peerPublicKey,session){
    ...
    const info=canonicalBytes({v:PROTOCOL_VERSION,context:KEY_CONTEXT,sessionId:String(session.sessionId),pairingId:String(session.pairingId),pairingBinding:base64UrlEncode(session.pairingBinding)});
  ...
  class ReplayGuard{
    ...
    check(frame){
      ...
      assert(Number(frame.sequence)>this.highest,'REPLAY','Sequenz ist nicht monoton steigend.');return id;
    }
  ...
  class LiveSession{
    ...
    async reserve(){
      ...
      const proof=await buildJoinProof(data=>this.sign(data),this.session.sessionId,this.session.sessionSalt);await this.relay.arm(this.session.code,this.token,proof);this.armExpiry();await this.connect();return this.session;
    }
    async join(code){
      ...
      this.closed=false;const challenge=await this.relay.challenge(String(code).replace(/\D/g,''));const details=normalizeSession(challenge,String(code).replace(/\D/g,''));this.session={...details,pairingId:this.pairingId};const proof=await buildJoinProof(data=>this.sign(data),this.session.sessionId,this.session.sessionSalt);const joined=await this.relay.join(this.session.code,proof);this.token=String(joined.patientToken||joined.token||'');assert(this.token,'RELAY_AUTH','Patienten-Sitzung konnte nicht autorisiert werden.');this.armExpiry();await this.connect();return this.session;
    }
    async connect(){
      assert(this.session&&!this.closed,'SESSION_REQUIRED','Keine aktive Sitzung.');if(Date.now()>=new Date(this.session.expiresAt).getTime())throw failure('SESSION_EXPIRED','Sitzung ist abgelaufen.');
      const socket=await this.transport.openSocket(this.session,this.role,this.token);this.socket=socket;this.bindSocket(socket);this.setStatus('connected');this.sendRaw({type:'auth',v:PROTOCOL_VERSION,role:this.role,token:this.token});
    ...
    sendRaw(value){assert(this.socket&&this.socket.readyState!==3,'SOCKET_CLOSED','Socket ist geschlossen.');const text=JSON.stringify(value);assert(text.length<=MAX_FRAME_BYTES,'FRAME_TOO_LARGE','Control-Frame ist zu groß.');this.socket.send(text);}
    async beginKeyExchange(){
      assert(this.session,'SESSION_REQUIRED','Keine aktive Sitzung.');let pair;
      if(this.cryptoBridge){
        ...
        this.nativePrivateHandle=pair&& (pair.privateKeyHandle||pair.handle||pair.privateKey);this.publicKey=String(pair&&pair.publicKey||'');assert(this.nativePrivateHandle&&this.publicKey,'NATIVE_CRYPTO_REQUIRED','KGGLiveKey-Bridge lieferte kein flüchtiges ECDH-Paar.');
      }else{pair=await createEphemeralKeyPair();this.privateKey=pair.privateKey;this.publicKey=pair.publicKey;}
      const body={v:PROTOCOL_VERSION,type:'key_hello',sessionId:this.session.sessionId,role:this.role,publicKey:this.publicKey};const signature=base64UrlEncode(await this.sign(canonicalBytes(body)));this.sendRaw({...body,signature});
    }
    async receiveRaw(raw){
      if(!raw)return;let value;try{value=typeof raw==='string'?JSON.parse(raw):raw;}catch(err){this.failClosed('INVALID_FRAME');return;}
      try{
        ...
        if(this.nativeSession){validateEnvelopeShape(value,this.peerRole);const guardId=this.guard.check(value);const result=await this.cryptoBridge.decryptFrame({sessionId:this.session.sessionId,role:this.role,expectedSender:this.peerRole,frame:value,sessionHandle:this.nativeSession});const payload=result&&result.payload||result;validateInner(payload);this.guard.commit(value,guardId);this.acceptedFrames+=1;await this.receiveInner(payload,value);}else{const result=await decryptEnvelope(this.key,this.session.sessionId,this.peerRole,value,this.guard);this.acceptedFrames+=1;await this.receiveInner(result.payload,result.frame);}
      }catch(err){this.failClosed(err.code||'FRAME_REJECTED');}
    }
    async receiveControl(value){
      assert(CONTROL_TYPES.has(value.type),'INVALID_CONTROL','Unbekannter Control-Frame.');
      ...
      const signed={v:value.v,type:value.type,sessionId:value.sessionId,role:value.role,publicKey:value.publicKey};const expected=await this.sign(canonicalBytes(signed));assert(equalBytes(expected,base64UrlDecode(value.signature)),'KEY_PEER_HMAC','Peer-HMAC ist ungültig.');
      ...
      if(!this.key&&!this.nativeSession){const binding=await buildPairingBinding(data=>this.sign(data),this.pairingId);if(this.cryptoBridge){assert(typeof this.cryptoBridge.deriveSessionKey==='function','NATIVE_CRYPTO_REQUIRED','KGGLiveKey-Bridge unterstützt keine Sitzungsschlüsselableitung.');const result=await this.cryptoBridge.deriveSessionKey({curve:'P-256',sessionId:this.session.sessionId,sessionSalt:this.session.sessionSalt,pairingId:this.pairingId,pairingBinding:base64UrlEncode(binding),privateKeyHandle:this.nativePrivateHandle,peerPublicKey:value.publicKey,role:this.role});this.nativeSession=result&&(result.sessionHandle||result.keyHandle||result.handle||result);assert(this.nativeSession,'NATIVE_CRYPTO_REQUIRED','KGGLiveKey-Bridge lieferte keinen Sitzungsschlüssel.');}else this.key=await deriveSessionKey(this.privateKey,value.publicKey,{...this.session,pairingBinding:binding});this.setStatus('ready',{keyReady:true});await this.flushQueue();await this.onReady(this.status());}
    }
    async send(type,payload){
      assert(!this.closed,'SESSION_CLOSED','Sitzung ist beendet.');assert(this.key||this.nativeSession,'KEY_NOT_READY','Sitzungsschlüssel ist noch nicht bereit.');assert(INNER_TYPES.has(type),'INVALID_MESSAGE_TYPE','Nachrichtentyp ist nicht erlaubt.');
      const inner={type,...(payload||{})};assertSynthetic(this.config,inner);if(type!=='close')assert(this.sendSequence<MAX_DATA_FRAMES,'QUOTA','Sitzungs-Frame-Limit erreicht.');const sequence=++this.sendSequence;let frame;if(this.nativeSession){assert(typeof this.cryptoBridge.encryptFrame==='function','NATIVE_CRYPTO_REQUIRED','KGGLiveKey-Bridge unterstützt keine Frame-Verschlüsselung.');const result=await this.cryptoBridge.encryptFrame({v:PROTOCOL_VERSION,sessionId:this.session.sessionId,role:this.role,sender:this.role,sequence,payload:inner,sessionHandle:this.nativeSession});frame=result&&result.frame||result;validateEnvelopeShape(frame,this.role);}else frame=await encryptEnvelope(this.key,this.session.sessionId,this.role,sequence,inner);assert(!this.sentNonces.has(frame.nonce),'NONCE_REUSE','Nonce wurde wiederverwendet.');this.sentNonces.add(frame.nonce);await this.queue.put(this.session.sessionId,frame,new Date(this.session.expiresAt).getTime());
      if(this.socket&&this.socket.readyState!==3){try{this.sendRaw(frame);await this.queue.remove(String(this.session.sessionId)+'|'+String(frame.sender)+'|'+String(frame.sequence));}catch(err){this.setStatus('offline');}}
      else this.setStatus('offline');return {messageId:frame.messageId,sequence:frame.sequence};
    }
    async flushQueue(){if(!this.socket||this.socket.readyState===3||(!this.key&&!this.nativeSession)||!this.session)return;const items=await this.queue.list(this.session.sessionId);for(const item of items){try{this.sendRaw(item.frame);await this.queue.remove(item.id);}catch(err){this.setStatus('offline');break;}}}
    ...
    async sendTrainingEvents(events,basePlanRevision){assert(this.role===ROLE_PATIENT,'ROLE_INVALID','Nur Patient:innen senden Trainingsergebnisse.');const value={synthetic:this.config.mode==='test',basePlanRevision:String(basePlanRevision||''),events:Array.isArray(events)?events:[]};validateTrainingEvents(value);return this.send('training_events',value);}
    ...
    async close(options){
      ...
      try{if(this.session&&this.token&&this.relay.close)await this.relay.close(this.session.code,this.token);}catch(err){}
      clearTimeout(this.timer);if(this.session)await this.queue.clear(this.session.sessionId).catch(()=>{});try{if(this.socket&&this.socket.close)this.socket.close();}catch(err){}this.closed=true;this.socket=null;this.key=null;this.nativeSession=null;this.nativePrivateHandle=null;this.privateKey=null;this.publicKey='';this.setStatus('closed',{reason});
    }
    async reconnect(){assert(!this.closed&&this.session,'SESSION_CLOSED','Sitzung ist beendet.');assert(Date.now()<new Date(this.session.expiresAt).getTime(),'SESSION_EXPIRED','Sitzung ist abgelaufen.');await this.connect();return this.status();}
  }
