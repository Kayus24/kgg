...
const versionMatch=worker.match(/const APP_VERSION = '([0-9]+)';/);
assert(versionMatch,'service worker APP_VERSION is missing');
...
assert((index.match(new RegExp(`patient-version-label\\.js\\?v=${appVersion}`,'g'))||[]).length===1,`index version label is not v${appVersion}`);
...
assert(new RegExp(`const APP_VERSION = '${appVersion}';`).test(worker)&&new RegExp(`const CACHE_NAME = 'kgg-handyplan-v${appVersion}-[a-z0-9-]+';`).test(worker),'service worker version markers are inconsistent');
