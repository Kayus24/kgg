...
r=api.removePlanState({plans:[A,B,C],active:0,activePlanId:'b'},1);assert(r.ok&&r.activeRemoved&&r.state.activePlanId==='c','stable active plan id fallback failed');
...
const keys=api.planStorageKeys(A);assert(keys.length===3&&keys.every(k=>/^kgg-v7-/.test(k)),'v7 plan keys not isolated');
